...
function bpe_send_approve_mail( $event )
{
	...
	$message = sprintf( __( "Hello,\n\nan event is waiting to be approved.\n\nName:\n%s\n\nDescription:\n%s\n\nLocation:\n%s\n\nPlease follow the link below to deal with this event:\n%s", 'events' ), bpe_get_event_name( $event ), bpe_get_event_description_excerpt_raw( $event ), bpe_get_event_location( $event ), admin_url( 'admin.php?page='. EVENT_FOLDER .'-approve' ) );
