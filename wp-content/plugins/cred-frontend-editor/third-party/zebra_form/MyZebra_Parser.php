...
class MyZebra_Tokenizer
{
    ...
    public static function makeToken($tok, $force_type=null)
    {
        $token=(object)array(
            'val'=>$tok, 
        ...
        switch ($force_type)
        {
            ...
            default:
                ...
                // date token
                else if (self::isDateInstance($tok))
                {
                    $token->type=self::$_TOKEN_TYPES['DATE'];
                    $token->isDate=true;
                }
                // array token
                else if (self::isArrayInstance($tok))
                {
                    $token->type=self::$_TOKEN_TYPES['ARRAY'];
                    $token->isArray=true;
                }
                // regex token
                else if (self::isRegExpInstance($tok))
                {
                    $token->type=self::$_TOKEN_TYPES['REGEX'];
                    $token->isRegex=true;
                }
...
class MyZebra_Parser
{
    ...
    private function _getVariable($strVarName, &$varArr)
    {
        if (!isset($varArr))
            $varArr=$this->arrVars;
