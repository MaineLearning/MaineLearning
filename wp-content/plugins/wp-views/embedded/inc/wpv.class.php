...
class WP_Views{
    ...
    function init(){
        ...
        add_action('wp_ajax_wpv_get_post_filter_summary', 'wpv_ajax_get_post_filter_summary');
    ...
    */
    ...
    function render_view($id){
        ...
        $css = get_post_meta($id, '_wpv_css', true);
        ...
        */
        ...
        if(!isset($processed_views[$view_caller_id][$id]) || 0 === $view_caller_id){

            

            $processed_views[$view_caller_id][$id] = true; // mark view as processed for this post
            ...
            if(!empty($view)){
                ...
                if(preg_match('#\<wpv-loop(.*?)\>(.*)</wpv-loop>#is', $post_content, $matches)) {
                    ...
                    $this->post_query = wpv_filter_get_posts($id);
            ...
            $processed_views[$view_caller_id][$id] = $out; // save output to be used later
            ...
            if($processed_views[$view_caller_id][$id] !== true){

                $out .= $processed_views[$view_caller_id][$id]; // use output from cache
...
function render_view($args) {
    ...
    if ($id) {
        ...
        return $WP_Views->render_view_ex($id);
