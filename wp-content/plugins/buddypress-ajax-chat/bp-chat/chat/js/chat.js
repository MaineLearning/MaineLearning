...
var ajaxChat = {
	...
	blinkOnNewMessage: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		if(this.lastID && !this.channelSwitch && userID != this.userID) {
	...
	blinkUpdate: function(blinkStr) {
		o = parent.document.getElementById('bp-adminbar-chat-menu');
		p = parent.document;
		if(!this.originalDocumentTitle) {
			this.originalDocumentTitle = p.title;
			this.originalDocumentMenu = o.innerHTML;
		}
		if(!arguments.callee.blink && o != null) {
			o.innerHTML = '[@ ] '+blinkStr+' - '+this.originalDocumentMenu;
			p.title = '[@ ] '+blinkStr+' - '+this.originalDocumentTitle;
			document.title = '[@ ] '+blinkStr+' - '+this.originalDocumentTitle;
			arguments.callee.blink = 1;
		} else if(arguments.callee.blink > this.settings['blinkIntervalNumber'] + 20) {
			clearInterval(this.blinkInterval);
			o.innerHTML = this.originalDocumentMenu;
			p.title = this.originalDocumentTitle;
