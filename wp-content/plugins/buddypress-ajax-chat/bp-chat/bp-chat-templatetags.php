 ...
 *
 * The chat template class below would allow you do the following in the template file:
 *
 * 	<?php if ( bp_get_chat_has_items() ) : ?>
 *
 *		<?php while ( bp_get_chat_items() ) : bp_get_chat_the_item(); ?>
 *
 *			<p><?php bp_get_chat_item_name() ?></p>
 *
 *		<?php endwhile; ?>
 *		
 *	<?php else : ?>
...
class BP_Chat_Template {
	...
	function bp_chat_template( $user_id, $type, $per_page, $max ) {
		global $bp;
		 ...
		 * For chat on the groups template loop, you can fetch groups by "newest", "active", "alphabetical"
		...
		// switch ( $type ) {
		// 	case 'newest':
		// 		$this->items = bp_chat_get_newest( $user_id, $this->pag_num, $this->pag_page );
		// 		break;
		// 
		// 	case 'popular':
		// 		$this->items = bp_chat_get_popular( $user_id, $this->pag_num, $this->pag_page );				
		// 		break;
		// 
		// 	case 'alphabetical':
		// 		$this->items = bp_chat_get_alphabetical( $user_id, $this->pag_num, $this->pag_page );				
		// 		break;
		// }
...
function bp_chat_has_items( $args = '' ) {
	 ...
	 * 
	 * e.g. bp_get_chat_has_items( 'per_page=10&max=50' );
	 */
	
	/***
	 * Set the defaults for the parameters you are accepting via the "bp_get_chat_has_items()"
	 * function call 
	 */
	$defaults = array(
		'user_id' => false,
	...
	$items_template = new BP_Chat_Template( $user_id, $type, $per_page, $max );
		
	return $items_template->has_items();
}

function bp_chat_the_item() {
	global $items_template;
	return $items_template->the_item();
}

function bp_chat_items() {
	global $items_template;
	return $items_template->user_items();
}

function bp_chat_item_name() {
	echo bp_chat_get_item_name();
}
	/* Always provide a "get" function for each template tag, that will return, not echo. */
	function bp_chat_get_item_name() {
		global $items_template;
		echo apply_filters( 'bp_chat_get_item_name', $items_template->item->name ); // Chat: $items_template->item->name;
	}
	
function bp_chat_item_pagination() {
	echo bp_chat_get_item_pagination();
}
	function bp_chat_get_item_pagination() {
		global $items_template;
		return apply_filters( 'bp_chat_get_item_pagination', $items_template->pag_links );
	}
