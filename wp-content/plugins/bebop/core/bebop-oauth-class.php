...
class bebop_oauth_token {
	...
	*/
	function to_string() {
		return 'oauth_token=' . bebop_oauth_util::urlencode_rfc3986( $this->key ) . '&oauth_token_secret=' . bebop_oauth_util::urlencode_rfc3986( $this->secret );
	}
...
class bebop_oauth_server {
	...
	*/
	private function get_signature_method( $request ) {
		$signature_method = $request instanceof bebop_oauth_request ? $request->get_parameter( 'oauth_signature_method' ) : NULL;
		if ( ! $signature_method ) {
	...
	*/
	private function get_consumer( $request ) {
		$consumer_key = $request instanceof bebop_oauth_request ? $request->get_parameter( 'oauth_consumer_key' ) : NULL;
		if ( ! $consumer_key ) {
	...
	*/
	private function check_timestamp( $timestamp ) {
		if( ! $timestamp )
			throw new bebop_oauth_exception( 'Missing timestamp parameter. The parameter is required' );
		// verify that timestamp is recentish
		$now = time();
		if ( abs( $now - $timestamp ) > $this->timestamp_threshold ) {
			throw new bebop_oauth_exception( "Expired timestamp, yours $timestamp, ours $now" );
		}
	}
	/**
