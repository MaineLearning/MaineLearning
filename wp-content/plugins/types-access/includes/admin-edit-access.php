...
function wpcf_access_admin_edit_access($enabled = true) {
    ...
    // Types
    $types = wpcf_get_active_custom_types();
    ...
    foreach ($types as $type_slug => $type_data) {
        // no label, bypass
        // same plural and singular names, bypass, else problems
        if (!wpcf_is_object_valid('type', $type_data)) {
    ...
    // Taxonomies
    $taxonomies = wpcf_get_active_custom_taxonomies();
    ...
    foreach ($taxonomies as $tax_slug => $tax_data) {
        // no label, bypass
        // same plural and singular names, bypass, else problems
        if (!wpcf_is_object_valid('taxonomy', $tax_data)) {
...
//                                implode(', ', $txt), $last_element);
    ...
    $link_to_manual = '<a href="http://wp-types.com/documentation/user-guides/#Access" title="'
            ...
            . 'class="wpcf-access-link-to-manual" style="display:block;font-weight:bold;background-image: url(\'' . WPCF_EMBEDDED_RELPATH . '/common/res/images/question.png\');background-repeat: no-repeat;text-indent: 18px;">'
            . __('Access Manuals &raquo;') . '</a>';

    echo $link_to_manual . $access_notices . $shortmenus . $output;
}
...
function wpcf_access_reassign_role_form($role) {
    ...
    if (!empty($users)) {
        ...
        $output .= wpcf_access_admin_roles_dropdown(wpcf_get_editable_roles(),
                'wpcf_reassign', array(),
