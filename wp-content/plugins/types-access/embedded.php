...
function wpcf_access_types_caps_predefined() {
    $modes = array(
        ...
        /*'edit_comments' => array(
            'title' => __('Edit Comments', 'wpcf_access'),
            'role' => 'author',
            'predefined' => 'edit_comments',
        )*/
    );
    return $modes;
}
 ...
 * @return type 
 */
function wpcf_get_editable_roles() {
    global $wpcf_access;
...
*   Auxilliary function
*/
function wpcf_object_to_array($data) {
    if (is_array($data) || is_object($data)) {
        $result = array();
        foreach ($data as $key => $value) {
            $result[$key] = wpcf_object_to_array($value);
        }
        return $result;
    }
    return $data;
