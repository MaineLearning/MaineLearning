		...
		if( $cant_publish_event || $cant_publish_recurring_event ){
		    if( $EM_Event->is_published() && !$EM_Event->previous_status ){
				...
				if( $approvals_count == 1 ){
		        	$subject = $EM_Event->output(get_option('dbem_event_approved_email_subject'), 'email');
		        	$body = $EM_Event->output(get_option('dbem_event_approved_email_body'), 'email');
				}else{
		        	$subject = $EM_Event->output(get_option('dbem_event_reapproved_email_subject'), 'email');
		        	$body = $EM_Event->output(get_option('dbem_event_reapproved_email_body'), 'email');				    
				} 
				...
				if( $approvals_count > 0 ){
					$subject = $EM_Event->output(get_option('dbem_event_resubmitted_email_subject'));
					$message = $EM_Event->output(get_option('dbem_event_resubmitted_email_body'));
				}else{
