	...
	function TI_Obj($atts=null) {
		...
		foreach ($obj_atts as $key => $value ) {
			...
			if (isset($atts[$key]))
				...
				$this->$key = (isset($defaults[$key])) ? $defaults[$key] : null;
	...
	*/
	...
	function update($atts=null) {
		...
		foreach($obj_atts as $key => $value) {
			...
			if (substr($key, 0, 1) != '_' && isset($atts[$key]))
	...
	*/
	...
	function get($table, $id) {
		...
		if ($result === false)
			...
			return new WP_Error('fatal', 'Error during get() on table $table');
	...
	*/
	...
	function get_list($table) {
		...
		if ($results === false)
			...
			return new WP_Error('fatal', 'Error during get_list() on table $table');
	...
	function save($table) {
		...
		if (!$this->_id) {
			...
			if ($result === false)
				...
				return new WP_Error('fatal', 'Error during insert on auto-numbered table $table');
			...
			if ($result === false)
				...
				return new WP_Error('fatal', 'Error during insert on user-keyed table $table');
