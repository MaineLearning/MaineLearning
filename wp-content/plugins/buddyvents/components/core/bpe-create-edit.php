...
function bpe_create_event_action()
{
	...
	if( isset( $_POST['save-event'] ) )
	{
		...
		if( count( bpe_get_config( 'completed_create_steps' ) ) == count( bpe_get_config( 'creation_steps' ) ) && bpe_get_config( 'current_create_step' ) == array_pop( array_keys( bpe_get_config( 'creation_steps' ) ) ) )
		{
			...
			if( bpe_get_option( 'approve_events' ) == true && ! is_admin() || bpe_get_displayed_event( 'group_approved' ) == false && ! is_admin() )
			{
				bpe_send_approve_mail( bpe_get_displayed_event() );

				do_action( 'bpe_saved_new_event_approve', bpe_get_displayed_event() );
			...
			else
			{
				...
				do_action( 'bpe_saved_new_event', bpe_get_displayed_event() );
				
				bpe_add_message( __( 'Your event was successfully published!', 'events' ) );
				bp_core_redirect( bpe_create_redirect_success_url( bpe_get_displayed_event( 'slug' ) ) );
			}
